...
- 官方文字帮助文档统一沉淀在 `configs/official-docs.json`，教程通过 `configs/tutorials.json` 的 `officialDocIds` 关联。官方文档只用来校准功能范围、流程顺序、平台覆盖、点数说明和客户口径；正式画面、红框、鼠标、下拉框和发布确认位置仍以真实后台截图和审片板为准。如果官方文档和当前页面冲突，画面按当前页面，口播写成“以页面展示为准”，不要照搬旧文档。
