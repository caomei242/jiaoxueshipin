...
- `功能点分组 / AI视频 / AI批量生成视频`：先作为独立教程项目登记，状态为 `待采集`；可以点击 `开始制作保姆教程` 创建独立输出目录、采集清单和制作板，但必须采集真实截图后才能开放一键出片。
- `玩法组合 / AI优化图到AI批量生成视频`：跳转到现有玩法组合审片台。

`待采集` 的功能点不能显示成死路。Hub 必须至少提供 `开始制作保姆教程`，用于创建制作工作区和拍摄清单；`一键生成当前教程` 只有在真实截图和正式审片台齐备后才开放。
