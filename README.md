...
多 Tab Hub 是本地统一入口，用来把「单功能保姆教程」和「玩法组合视频」分开管理。它不替代各子功能的审片台，而是负责按教程目录进入对应审片、生成当前教程、打开视频和定位输出目录。
...
- `功能点分组`：单个功能点的保姆教程，左侧再按 `AI图片`、`AI视频` 等能力模块分组。
- `玩法组合`：多个功能串成一条客户能理解的玩法链路，后续可以继续新增多个玩法卡片。

当前已登记的条目：

- `功能点分组 / AI图片 / AI优化商品图`：跳转到现有保姆教程审片台。
- `功能点分组 / AI视频 / AI批量生成视频`：先作为独立教程项目登记，状态为 `待采集`；可以点击 `创建采集清单` 创建独立输出目录、采集清单和口播草稿，但必须采集真实截图后才能开放正式审片和一键出片。
- `玩法组合 / AI优化图到AI批量生成视频`：跳转到现有玩法组合审片台。

`待采集` 的功能点不能显示成死路。Hub 必须至少提供 `创建采集清单` 或等价入口，用于创建制作工作区、拍摄清单和口播草稿；`一键生成当前教程` 只有在真实截图和正式审片台齐备后才开放。

## 官方帮助文档参考

官方文字帮助文档沉淀在 `configs/official-docs.json`，教程目录通过 `officialDocIds` 关联。它只负责校准功能范围、流程顺序、平台覆盖、点数说明和客户口径；正式视频画面仍必须来自真实后台截图和审片板。

当前已接入：

- `多店AI优化商品图-操作教程`：`https://www.yuque.com/huanleguang/hlgddgl/xgl71af4r29ngxpf`

生成脚本时，`scripts/generate_scripts.mjs` 会把匹配到的官方文档摘要写入 `scripts/tutorial-scripts.json`。如果官方文档和当前后台页面有差异，画面按当前页面，口播里用“以页面展示为准”的口径，不直接照搬旧文档。
